...
class Lexer:
	...
	def make_identifier(self):
		...
		if id_str == 'return':
			return Token(TT_RETURN, pos_start=pos_start, pos_end=self.pos)
...
class Parser:
	...
	def func_def(self):
		...
		if self.current_tok.type != TT_RETURN:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected 'return'"
			))
...
class Number(Value):
	...
	def is_true(self):
		return String("true") if self.value != 0 else String("false")
	...
	def dived_by(self, other):	
		if isinstance(other, Number):
			try:
				...
				return None, RTError(
					other.pos_start, other.pos_end,
					f"Element at this index could not be retrieved from the list",
					self.context
				)
		else:
			return None, Value.illegal_operation(self, other)
...
class BaseFunction(Value):
	...
	def check_and_populate_args(self, arg_names, args, exec_ctx):
		res = RTResult()
		res.register(self.check_args(args, arg_names))
		if res.error: return res
...
global_symbol_table.set("say", BuiltInFunction.print)
global_symbol_table.set("sayInt", BuiltInFunction.print_ret)
global_symbol_table.set("takeInp", BuiltInFunction.input)
global_symbol_table.set("takeInpInt", BuiltInFunction.input_int)
